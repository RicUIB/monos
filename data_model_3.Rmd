...
ggtree(tree15, layout='circular') %<+% primates15_mapping + 
    aes(color="red") + geom_tippoint(aes(size=0.01), alpha=.6) + 
    geom_tiplab(aes(
      label=c(arrange(primates15_mapping,mDag_Idn)$Labels,as.character(16:29)),
      offset=.1),size=2,colour="black")+
...
ggtree(tree15,layout='slanted') %<+% primates15_mapping + 
  ...
  geom_tiplab(aes(label=c(arrange(primates15_mapping,mDag_Idn)$Labels,as.character(16:29))),size=3 ,angle=-40,colour="black",offset=-0.01)+
...
ggtree(tree5, layout='circular') %<+% index_hominoides + 
    aes(color="red") + geom_tippoint(aes(size=0.1), alpha=.6) + 
    geom_tiplab(aes(
      label=c(arrange(index_hominoides,idn)$Labels, as.character(6:9))),
      offset=.1,size=3,colour="black")+
  theme(legend.position = "none")+
  theme(plot.margin=margin(60,60,60,60))


ggtree(tree5,layout='slanted') %<+%  index_hominoides+ 
  aes(color="red")+ geom_tippoint() +
...
```{r}
labs=c(MBBH$mDag_Id,
       paste(combn(MBBH$mDag_Id,2)[1,],combn(MBBH$mDag_Id,2)[2,],sep="_"),
       paste(combn(MBBH$mDag_Id,3)[1,],combn(MBBH$mDag_Id,3)[2,],
             combn(MBBH$mDag_Id,3)[3,],sep="_"),
       paste(combn(MBBH$mDag_Id,4)[1,],combn(MBBH$mDag_Id,4)[,2],
             combn(MBBH$mDag_Id,4)[3,],combn(MBBH$mDag_Id,4)[,4],
             sep="_"),
       paste(combn(MBBH$mDag_Id,5),collapse="_"))
tb=tibble(n=c(one,two,three,four,five),labs,order=1:31,`number species`=rep(c(1,2,3,4,5),times=c(5,10,10,5,1)))
tb=arrange(tb,order,n)
ggplot(data=tb, aes(x=reorder(labs,order), y=n)) + geom_bar(stat="identity", fill=tb$`number species`)+ coord_flip()+
  labs(title = "Number of MBB's by several cores", x = "Species", y = "Number of MBB's")+
  scale_x_discrete(limits = arrange(tb,order,n)$labs)
```
